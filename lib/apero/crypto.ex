defmodule Apero.Crypto do
  ...
  def sha256(data) when is_binary(data), do: Apero.Crypto.Hash.sha256(data)

  @deprecated "Use Apero.Crypto.Hash.sha512/1 instead"
  @doc "SHA-512 hash (hex encoded)."
  @spec sha512(binary()) :: binary()
  def sha512(data) when is_binary(data), do: Apero.Crypto.Hash.sha512(data)
  ...
  def md5(data) when is_binary(data), do: Apero.Crypto.Hash.md5(data)
  ...
  def hmac(secret, data) when is_binary(secret) and is_binary(data),
    do: Apero.Crypto.Hash.hmac(secret, data)
  ...
  def encrypt(plaintext, key \\ nil) when is_binary(plaintext),
    do: Apero.Crypto.Cipher.encrypt(plaintext, key)
  ...
  def decrypt(encoded, key) when is_binary(encoded) and is_binary(key),
    do: Apero.Crypto.Cipher.decrypt(encoded, key)
  ...
  def encrypt_chacha20(plaintext, key) when is_binary(plaintext) and byte_size(key) == 32,
    do: Apero.Crypto.Cipher.encrypt_chacha20(plaintext, key)
  ...
  def decrypt_chacha20(encoded, key) when is_binary(encoded) and byte_size(key) == 32,
    do: Apero.Crypto.Cipher.decrypt_chacha20(encoded, key)
  ...
  def stream_init(key) when byte_size(key) == 32,
    do: Apero.Crypto.Cipher.stream_init(key)
  ...
  def stream_encrypt({state, iv}, chunk),
    do: Apero.Crypto.Cipher.stream_encrypt({state, iv}, chunk)
  ...
  def stream_finalize(state),
    do: Apero.Crypto.Cipher.stream_finalize(state)
  ...
  def decrypt_ctr(ciphertext, key, iv) when byte_size(key) == 32 and byte_size(iv) == 16,
    do: Apero.Crypto.Cipher.decrypt_ctr(ciphertext, key, iv)
  ...
  def generate_ecdh_keypair,
    do: Apero.Crypto.Key.generate_ecdh_keypair()
  ...
  def compute_ecdh_secret(my_private, peer_public),
    do: Apero.Crypto.Key.compute_ecdh_secret(my_private, peer_public)
  ...
  def pbkdf2(password, salt, opts \\ []) when is_binary(password) and is_binary(salt),
    do: Apero.Crypto.Key.pbkdf2(password, salt, opts)
  ...
  def argon2id(password, salt, opts \\ []),
    do: Apero.Crypto.Key.argon2id(password, salt, opts)
  ...
  def generate_rsa_keypair,
    do: Apero.Crypto.Key.generate_rsa_keypair()
  ...
  def generate_key,
    do: Apero.Crypto.Random.generate_key()
  ...
  def random_hex(bytes \\ 32),
    do: Apero.Crypto.Random.random_hex(bytes)
  ...
  def random_token(bytes \\ 32),
    do: Apero.Crypto.Random.random_token(bytes)
  ...
  def random_password(length \\ 24, opts \\ []),
    do: Apero.Crypto.Random.random_password(length, opts)
  ...
  def secure_compare(a, b) when is_binary(a) and is_binary(b) and byte_size(a) == byte_size(b),
    do: Apero.Crypto.Random.secure_compare(a, b)
